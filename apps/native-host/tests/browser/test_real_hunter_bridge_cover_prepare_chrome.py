...
def _hunter_modules(monkeypatch: pytest.MonkeyPatch):
    ...
    from app import api
    from app import application_artifact_transport_v2 as bridge
...
def test_real_signed_hunter_bridge_uploads_bound_resume_and_cover_before_input(
    tmp_path,
    monkeypatch,
):
    ...
    resume = {
        "engine": "NATIVE_V5",
        "version_id": "synthetic-resume",
        "artifact_id": "synthetic-resume",
        "artifact_reference": "hunter-native-resume://synthetic/pdf",
        "artifact_sha256": hashlib.sha256(RESUME).hexdigest(),
        "filename": "resume.pdf",
        "mime_type": "application/pdf",
    }
    cover = {
        "version": "munshi-cover-letter-artifact-v1",
        "artifact_id": "synthetic-cover",
        "artifact_reference": "hunter-cover-letter://synthetic/pdf",
        "artifact_sha256": hashlib.sha256(COVER).hexdigest(),
    ...
    hunter_plan = {
        ...
        "snapshot": {
            "owner_binding": {"tenant_id": "tenant-a", "user_id": "member-a"},
            "resume": resume,
            "cover_letter": cover,
            "submission_authority": False,
        },
    ...
    monkeypatch.setattr(
        bridge.application_plan_v2,
        "executable_plan",
        lambda _id: hunter_plan,
    )
    monkeypatch.setattr(
        bridge.native_resume_artifact_v5,
        "artifact_bytes",
        lambda _id: RESUME,
    )
    monkeypatch.setattr(
        bridge.cover_letter_artifact_v1,
        "artifact_bytes",
        lambda _id: COVER,
    )
    ...
    html = (
        ...
        "<input id='cover_letter' type='file' required>"
    ...
    with _hunter_server(api) as base_url:
        ...
        result = runner.run_once(worker_id="synthetic-worker")

    assert result is not None
    queue_record = queue.get(
        job_id=result.job_id,
        tenant_id="tenant-a",
        user_id="member-a",
    )
    assert result.job_state == "WAITING_INPUT", queue_record["last_error"]
    assert result.session_state == "NEEDS_INPUT", queue_record["last_error"]
    ...
    with db.connect() as connection:
        ...
        assert resume["artifact_sha256"] in evidence["evidence_json"]
        assert cover["artifact_sha256"] in evidence["evidence_json"]
        count_queries = (
