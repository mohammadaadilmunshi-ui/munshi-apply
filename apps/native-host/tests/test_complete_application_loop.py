...
def test_one_customer_approval_freezes_and_submits_once(loop):
    ...
    session = service.start_session(plan_id="application-plan-1")
    assert service.prepare_session(session_id=session.session_id, adapter=browser).state == "NEEDS_INPUT"
    ...
    assert service.prepare_session(session_id=session.session_id, adapter=browser).state == "READY_FOR_REVIEW"
    review = service.build_review(session_id=session.session_id)

    first = service.approve_and_submit(
        review_id=review["review_id"], idempotency_key="single-approval-1", adapter=browser
    )
