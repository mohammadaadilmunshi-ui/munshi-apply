...
- M1 is substantially implemented. The desktop editor now exposes a broad structured application profile, auto-saves ordinary facts, explicitly confirms protected facts, synchronizes encrypted profile state, and includes the encrypted résumé vault. Hosted-workspace field parity plus the final owner recovery-key drill with synthetic data remain release gates before real private-data migration.
- M2 includes all-frame aggregation and open Shadow DOM discovery. Broader custom-widget coverage, section/navigation semantics, validation-state modeling, and unusual dynamic controls remain incremental work.
- M3 is implemented for synchronized application reviews and explicit sensitive-answer approval. The mature evidence-backed resolver, contradiction handling, and knockout policy remain tied to M6 retrieval work.
- M4 is implemented for supported native controls with DOM verification. File controls and unsupported custom widgets remain manual; richer state waits and advanced interaction recipes remain to be added.
...
- M9 hardening is continuous; the final permission, accessibility, performance, backup/restore, security, privacy, and release audits remain future gates.
