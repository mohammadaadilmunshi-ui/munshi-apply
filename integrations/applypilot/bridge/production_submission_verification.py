...
Receipt identity is deterministic from the verified content (not a fresh UUID).
A worker re-run that reproduces the same browser observation, provider lookup,
authorization, and claim must produce the SAME ``receipt_id``/``receipt_digest``,
matching the existing ``synthetic-submission-receipt-<digest32>`` convention.
This is the §6 idempotency fix: identical facts MUST yield one logical receipt.
"""
...
def build_verified_receipt(*,authorization:Mapping[str,Any],claim:Mapping[str,Any],execution:Mapping[str,Any],provider_observation:Mapping[str,Any],verified_at:str|None=None)->dict[str,Any]:
    ...
    # Use a caller-supplied verified_at when provided so the receipt identity is
    # fully deterministic from verified content (no clock noise). Fall back to
    # the current UTC instant when the caller does not pin it.
    now = verified_at if isinstance(verified_at, str) and verified_at.strip() else datetime.now(UTC).isoformat().replace("+00:00", "Z")
    material={
    ...
    # Deterministic receipt id derived from the receipt digest, matching the
    # existing native ``synthetic-submission-receipt-<digest32>`` convention so
    # identical facts always produce one logical receipt.
    receipt_id = "production-receipt-" + receipt_digest[:32]
    material_with_id = {**material, "receipt_id": receipt_id}
    signature=hmac.new(_secret(),receipt_digest.encode(),hashlib.sha256).hexdigest()
    return {**material_with_id,"receipt_digest":receipt_digest,"signature":f"sha256={signature}"}
